...
def login():
    ...
    if driver.find_element_by_class_name("cart-checkbox"):
        driver.find_element_by_class_name("cart-checkbox").click()
...
def buy(buytime):
    while True:
        ...
        if now > buytime:
            try:
                # 点击结算按钮
                if driver.find_element_by_class_name("order-submit"):
                    driver.find_element_by_class_name("order-submit").click()
